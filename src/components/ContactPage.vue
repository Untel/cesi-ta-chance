...
<script>
  ...
  import { fromFrontToBack } from './../utils';
  ...
  export default {
    ...
    methods: {
      submit() {
        ...
        const candidate = {
          ...
          contactPlace: this.$services.contact.contactPlace,
          _id: nanoid(),
        };

        this.$api.post('candidate', fromFrontToBack(candidate))
          .then(() => {
            this.$services.snack.success('Enregistré');
            this.clear();
            this.submitting = false;
          })
          .catch(() => {
            this.$db.insert(candidate, (err) => {
              if (err) {
                this.$services.snack.error('Echec de l\'enregistrement dans la base locale');
              } else {
                this.$services.snack.warn(`Pas de connection internet détecté.<br/>L'utilisateur à été enregistré localement en attendant une future synchronisation.`);
              }
              this.clear();
              this.submitting = false;
            });
    ...
    computed: {
      ...
      phoneErrors() {
        ...
        if (!this.$v.contact.phone.required) errors.push('Le numéro de téléphone est requis');
        return errors;
      },
      birthdateErrors() {
        const errors = [];
      ...
      contactPlace() {
        return this.$services.contactPlace;
      }
    },
  };
</script>